...
void listAllFromNode(TrieNode *node, char *buffer, int depth) {
  if (node == NULL)
    return;
  ...
  for (int i = 0; i < 26; i++) {
    if (node->children[i] != NULL) {
      buffer[depth] = 'a' + i;
      listAllFromNode(node->children[i], buffer, depth + 1);
    }
  }
}
...
/*
 char word[100], description[256], prefix[100];
  char buffer[100];
  int choice;
